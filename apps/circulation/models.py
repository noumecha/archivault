...
class AuditLog(models.Model):
    """
    Journal d'audit complet — Traçabilité de toutes les actions du système.
    """
    ...
    action = models.CharField(max_length=30, choices=ActionAudit.choices, db_index=True)
    statut = models.CharField(max_length=10, choices=StatutAudit.choices, default=StatutAudit.SUCCESS, db_index=True)
    content_type = models.ForeignKey(ContentType, on_delete=models.SET_NULL, null=True, blank=True)
    object_id = models.PositiveIntegerField(null=True, blank=True)
    content_object = GenericForeignKey('content_type', 'object_id')
    objet_label = models.CharField(max_length=255, blank=True, help_text="Nom/Titre de l'objet au moment de l'action")
    details = models.JSONField(
        blank=True,
        null=True,
        help_text="Stocke les deltas (avant/après) ou des précisions textuelles."
    )
    ...
    def __str__(self):
        user_str = self.utilisateur if self.utilisateur else "Système/Anonyme"
        return f"[{self.timestamp:%d/%m/%Y %H:%M}] {user_str} - {self.get_action_display()} ({self.get_statut_display()}) -> {self.objet_label}"
