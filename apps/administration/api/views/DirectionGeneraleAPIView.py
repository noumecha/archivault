# apps/administration/api/views/DirectionGeneraleAPIView.py
...
from apps.circulation.services.audit_service import AuditService  # Import du service d'audit
from apps.circulation.models import ActionAudit, StatutAudit     # Import des enums d'audit
...
class DirectionGeneraleAPIView(DRFRoleRequiredMixin, BaseAPIView):
    ...
    def retrieve_action(self, request, pk=None, *args, **kwargs):
        ...
        if response.status_code == status.HTTP_200_OK:
            AuditService.log(
                ...
                obj=instance,
                details={"contexte": "vue_detail_direction_generale"}
            )
        else:
            AuditService.log(
                request,
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # OVERRIDE DES ACTIONS POUR AJOUTER LES PERMISSIONS ET L'AUDIT
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def delete_action(self, request, pk=None, *args, **kwargs):
        ...
        self.check_role_permission(request)
        instance = self.get_object()
        # Sauvegarde du nom de la direction avant la suppression physique en DB
        label_backup = f"[Direction Générale] {str(instance)}"
    ...
    def action_bulk_delete(self, request, *args, **kwargs):
        """Suppression en masse sécurisée par transaction."""
        self.check_role_permission(request)
        ...
        try:
            with transaction.atomic():
                ...
                # Log de masse avant l'exécution de la suppression
                AuditService.log(
                    request,
                    action=ActionAudit.SUPPRESSION_MASSE,
                    label=f"Suppression de {count} direction(s) générale(s)",
                ...
                return Response({
                    'success': True,
                    'message': f'{count} direction(s) générale(s) supprimée(s)',
                    'deleted_count': count
                })
